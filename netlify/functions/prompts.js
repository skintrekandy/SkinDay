...
const BIOSTIM = {
  sculptra: {
    ...
    avoid: 'this is collagen-driven SUPPORT, not filler SHAPE and not a beauty filter, so keep every feature outside soft-tissue volume identical to the original. These prohibitions are absolute and apply equally at every timeframe and every projection: a longer timeframe or stronger projection increases ONLY soft-tissue support and never relaxes any rule below. ' +
           ...
           'Volume-deficit floor (applies at every timeframe, including 12 months): the floor is for genuinely LEAN, well-supported faces only. If the face already shows good lateral support with minimal temple, lateral cheek, and lower-face volume loss, the result should stay very close to the original — do not invent improvements just to produce a visible change, and a longer timeframe is never a reason to add more volume than the face needs. When little deficit exists, the correct output may be nearly indistinguishable from the original. A round, full, or heavy face is the opposite case: that fullness is descent, not good support, so it must NOT be held at baseline by this floor and instead receives more lateral lift to draw the fullness up and outward and narrow the front. ' +
...
const VERSIONS = {
  ...
  cheeks: 'v2', tear_trough: 'v1', nasolabial_folds: 'v1', sculptra: 'v9', hdr: 'v1', timeline: 'v2'
};
