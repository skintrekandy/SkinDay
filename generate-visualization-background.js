...
exports.handler = async (event) => {
  ...
  try {
    ...
    const editParams = {
      ...
      prompt: finalPrompt,
      size: 'auto',
      // M11.1: Enhanced uses 'low' fidelity -- 'high' pixel-locks the input
      // so tightly the model produces near-null changes (confirmed by debug sheet
      // panel 3 being almost identical to original). Low loosens the constraint
      ...
      // Valid values: 'high' | 'low' (no 'medium' in this API).
      input_fidelity: isEnhancedJob ? 'low' : 'high',
      output_format: 'jpeg',
